...
namespace SevenStarAutoSell.Crawl.Common
{
    ...
    public class MQThreads : Singleton<MQThreads>
    {
        ...
        /// 服务器下发扫水命令
        /// </summary>
        public event PushCollectEventHandler PushCollectEvent;

        /// <summary>
        /// 服务器下发扫水命令
        /// </summary>
        public event PushCollectEventHandler SendCollectEvent;
        ...
        private void HandleKeyData_DealerReceive(string cmdText, string actionJsonResult)
        {
            ...
            //}           
            //else 
            if (cmdText == "PushCollect")
            {
                Collect col = JsonUtil.Deserialize<Collect>(actionJsonResult);
                PushCollectEvent?.Invoke(col);
            }
            else if(cmdText == "SendCollect")
            {
                Collect col = JsonUtil.Deserialize<Collect>(actionJsonResult);
                PushCollectEvent?.Invoke(col);
            }
    ...
    /// 服务器下发的扫水命令
    /// </summary>
    /// <param name="collect">扫水内容</param>
    public delegate void PushCollectEventHandler(Collect collect);
